...
class DatasetPtCrop(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        imgA_input = imgA * imgA_gt
        imgB_input = imgB * imgB_gt
        imgA_crop = cropImageByPoint(imgA_input, coorA, self.limit)
        imgB_crop = cropImageByPoint(imgB_input, coorB, self.limit)

        imgA_tensor = torch.FloatTensor(imgA_crop).unsqueeze(0)
        imgB_tensor = torch.FloatTensor(imgB_crop).unsqueeze(0)

        return imgA_tensor, imgB_tensor, os.path.basename(base_dir)
