...
sys.path.append("../")
sys.path.append("./")
from metrics.gd import GradientDifference1d, GradientDifference2d, GradientDifference3d
from metrics.function import spatial_filter_nd, normalized_cross_correlation

# https://github.com/yuta-hi/pytorch_similarity/blob/master/torch_similarity/modules/gradient_correlation.py
...
class GradientCorrelation1d(GradientDifference1d):
    ...
    def __init__(self,
        ...
        super().__init__(grad_method, gauss_sigma, gauss_truncate, return_map,
                         reduction)
    ...
    def forward(self, x, y):
        ...
        gc, gc_map = normalized_cross_correlation(x_grad, y_grad, True,
...
class GradientCorrelation2d(GradientDifference2d):
    ...
    def forward(self, x, y):
        ...
        # gradient correlation
        gc_u, gc_map_u = normalized_cross_correlation(x_grad_u, y_grad_u, True,
                                                      self.reduction, self.eps)
        gc_v, gc_map_v = normalized_cross_correlation(x_grad_v, y_grad_v, True,
                                                      self.reduction, self.eps)
...
class GradientCorrelation3d(GradientDifference3d):
    ...
    def forward(self, x, y):
        ...
        gc_v, gc_map_v = normalized_cross_correlation(x_grad_v, y_grad_v, True,
                                                      self.reduction, self.eps)
        gc_w, gc_map_w = normalized_cross_correlation(x_grad_w, y_grad_w, True,
                                                      self.reduction, self.eps)
