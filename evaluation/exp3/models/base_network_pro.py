...
class VTNAffineStem(nn.Module):
    def __init__(self, channels=16):
        ...
        self.bottleneck = nn.Sequential(
            LeakyReLU(0.1), nn.Conv3d(64 * channels, 16 * channels, 1, 1, 0),
            nn.InstanceNorm3d(16 * channels), LeakyReLU(0.1))

        self.fc_loc = nn.Sequential(
            Linear(channels * 48 * 16, 1024),
            ReLU(True),
            Dropout(0.3),
            Linear(1024, 256),
            ReLU(True),
    ...
    def forward(self, imgA, imgB, imgA_gt, imgB_gt):
        ...
        feature_A_pro = nn.AvgPool3d((x6_1_A.size(2), 1, 1))(x6_1_A).squeeze(2)
        feature_B_pro = nn.AvgPool3d((x6_1_B.size(2), 1, 1))(x6_1_B).squeeze(2)

        x_input = torch.cat((x6_1_A, x6_1_B), dim=1)
        # Affine transformation
        x_fc_input = self.bottleneck(x_input)
        xs = x_fc_input.view(imgA.size(0), -1)  # x6_1
        rt = self.fc_loc(xs)

        return rt, feature_A_pro, feature_B_pro
