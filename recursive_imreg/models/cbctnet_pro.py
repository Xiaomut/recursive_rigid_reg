...
class FeatureConcat(nn.Module):
    ...
    def forward(self, imgA, imgB, imgA_gt, imgB_gt):
        ...
        feature_A_pro = nn.AvgPool3d(
            (feature_A.size(2), 1, 1))(feature_A).squeeze(2)
        feature_B_pro = nn.AvgPool3d(
            (feature_B.size(2), 1, 1))(feature_B).squeeze(2)
...
if __name__ == "__main__":
    ...
    x2 = torch.rand(1, 8, 5, 5, 5).to(device)
    # net = FeatureConcat(8).to(device)
    # # print(net)
    # y = net(x1, x1, x1, x1)
    # print(y)
    m1 = nn.AdaptiveAvgPool3d((1, x2.size(-2), x2.size(-1)))
    m2 = nn.AvgPool3d((5, 1, 1))
    print(m2(x2).squeeze(2).shape)