...
class UNet3D(nn.Module):
    def __init__(self,
        ...
        encoder_layers.append(
            First3D(in_channels, conv_depths[0], conv_depths[0]))
        encoder_layers.extend([
            Encoder3D(conv_depths[i], conv_depths[i + 1], conv_depths[i + 1])
        ...
        decoder_layers.extend([
            Decoder3D(2 * conv_depths[i + 1], 2 * conv_depths[i],
                      2 * conv_depths[i], conv_depths[i])
            for i in reversed(range(len(conv_depths) - 2))
        ])
        decoder_layers.append(
            Last3D(conv_depths[1], conv_depths[0], out_channels))

        # encoder, center and decoder layers
        self.encoder_layers = nn.Sequential(*encoder_layers)
        self.center = Center3D(conv_depths[-2], conv_depths[-1],
    ...
    def forward(self, x, return_all=False):
        ...
        for dec_layer_idx, dec_layer in enumerate(self.decoder_layers):
            ...
            x_cat = torch.cat(
                [pad_to_shape(x_dec[-1], x_opposite.shape), x_opposite], dim=1)
...
def pad_to_shape(this, shp):
    ...
    if len(shp) == 4:
        ...
        pad = (0, shp[4] - this.shape[4], 0, shp[3] - this.shape[3], 0,
               shp[2] - this.shape[2])
...
if __name__ == "__main__":
    ...
    net = UNet3D(1, 1, conv_depths=(32, 64, 128, 256)).to(device)
    print(net)
    y = net(x)
    print(y.shape)